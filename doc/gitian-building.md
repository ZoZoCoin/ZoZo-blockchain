...
*Setup instructions for a Gitian build of Zozo Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Zozo
...
- [Building Zozo Core](#building-zozocoin-core)
...
Clone the git repositories for Zozo Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/zozocoinpay/zozocoin
```
...
in the Zozo Core repository under 'Fetch and build inputs' to install sources which require
...
To build Zozo Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Zozo Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zozocoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/zozocoinpay/zozocoin
...
For example:
```bash
URL=https://github.com/crowning-/zozocoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit zozocoin=${COMMIT} --url zozocoin=${URL} ../zozocoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zozocoin=${COMMIT} --url zozocoin=${URL} ../zozocoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zozocoin=${COMMIT} --url zozocoin=${URL} ../zozocoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the zozocoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../zozocoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/zozocoin.git
SIGPATH=/some/root/path/zozocoin-detached-sigs.git

./bin/gbuild --url zozocoin=${BTCPATH},signature=${SIGPATH} ../zozocoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zozocoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zozocoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/zozocoin-osx-build.assert
```
...
[zozocoin/gitian.sigs](https://github.com/zozocoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
