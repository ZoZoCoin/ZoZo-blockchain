...
Dtmi Core version 0.11.2 is now available from:

  https://dtmipay.io/downloads
...
Dtmi Core's block templates are now for version 3 blocks only, and any
mining software relying on its `getblocktemplate` must be updated in parallel
to use libblkmaker either version 0.4.2 or any version from 0.5.1 onward. If you
are solo mining, this will affect you the moment you upgrade Dtmi Core,
...
Dtmi Core 0.11.2 implements BIP 66, which introduces block version 3, and a
new consensus rule, which prohibits non-DER signatures. Such transactions have
been non-standard since Dtmi 0.8, but were technically still permitted
inside blocks.
...
installer (on Windows) or just copy over /Applications/Dtmi-Qt (on Mac) or
dtmid/dtmi-qt (on Linux).
