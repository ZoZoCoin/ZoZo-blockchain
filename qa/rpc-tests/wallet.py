...
class WalletTest (BitcoinTestFramework):
    ...
    def check_fee_amount(self, curr_balance, balance_with_fee, fee_per_byte, tx_size):
        ...
        if fee < target_fee:
            raise AssertionError("Fee of %s DTMI too low! (Should be %s DTMI)"%(str(fee), str(target_fee)))
        # allow the node's estimation to be at most 2 bytes off
        if fee > fee_per_byte * (tx_size + 2):
            raise AssertionError("Fee of %s DTMI too high! (Should be %s DTMI)"%(str(fee), str(target_fee)))
        return curr_balance
    ...
    def run_test (self):
        ...
        # Send 210 DTMI from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 1000 DTMI in block rewards plus fees, but
        ...
        # Send 100 DTMI with subtract fee from amount
        ...
        # Sendmany 100 DTMI with subtract fee from amount
