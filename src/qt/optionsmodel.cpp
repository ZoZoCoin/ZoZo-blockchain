...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::DTMI);
    ...
    if (!settings.contains("nPrivateSendAmount")) {
        ...
        if (!settings.contains("nAnonymizeDtmiAmount"))
            settings.setValue("nPrivateSendAmount", DEFAULT_PRIVATESEND_AMOUNT);
        else
            settings.setValue("nPrivateSendAmount", settings.value("nAnonymizeDtmiAmount").toInt());
    }
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in dtmi.cpp during shotdown to also remove the window positions
