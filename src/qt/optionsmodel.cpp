...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ZOZO);
    ...
    if (!settings.contains("nPrivateSendAmount")) {
        ...
        if (!settings.contains("nAnonymizeZozocoinAmount"))
            settings.setValue("nPrivateSendAmount", DEFAULT_PRIVATESEND_AMOUNT);
        else
            settings.setValue("nPrivateSendAmount", settings.value("nAnonymizeZozocoinAmount").toInt());
    }
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in zozocoin.cpp during shotdown to also remove the window positions
