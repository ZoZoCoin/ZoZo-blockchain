...
void CMasternode::Check(bool fForce)
{
	...
	{
		if (fDebug) LogPrintf("vlastPing.sigTime= %d\n", lastPing.sigTime);
		if (fDebug) LogPrintf("sigTime= %d\n", sigTime);
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        if(coins.vout[vin.prevout.n].nValue != 20000 * COIN) {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 20000 DTMI, masternode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 1000 DTMI tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 1000 DTMI tx -> 1 confirmation
