 ...
 *
 *   // network
 *   CZozocoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.zozocoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CZozocoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CZozocoinNetwork(UniValue objIn)
    {
...
// // can be under: ZozocoinNetwork
// //   -- signature requirements : Key1(User)
// class CZozocoinNetworkVariable : public CGovernanceObject
// {
// private:
