...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "DtmiNetwork") type = DtmiNetwork;
    if(strType == "DtmiNetworkVariable") type = DtmiNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == DtmiNetwork) s = "DtmiNetwork";
    if(type == DtmiNetworkVariable) s = "DtmiNetworkVariable";
