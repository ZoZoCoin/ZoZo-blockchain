...
*
*   Why is this needed?
*   - In Zozocoin some blocks are superblocks, which output much higher amounts of coins
...
void CMasternodePayments::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if(fLiteMode) return; // disable all Zozocoin specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f ZOZO\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
